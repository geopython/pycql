...
What is returned by the :func:`pycql.parser.parse` is the root
:class:`pycql.ast.Node` of the AST representation.

Inspection
----------

The easiest way to inspect the resulting AST is to use the
:func:`pycql.ast.get_repr` function, which returns a nice string
